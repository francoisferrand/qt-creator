...
QList<QTextLayout::FormatRange> CPPEditorWidget::getMacroUsagesForBlock(const QTextBlock &b)
{
    ...
    while (b.isValid() && it.hasNext()) {
        ...
        foreach (const SemanticInfo::Use &use, it.value()) {
			...
			case SemanticInfo::Use::Function:
				formatRange.format = m_functionFormat;
                break;

			case SemanticInfo::Use::Enum:
				formatRange.format = m_enumFormat;
				break;
