...
private:
    ...
    {
        bool ok;
        _result = QString::fromLatin1(tokenAt(ast->literal_token).spell()).toInt(&ok, 0);
        if (!ok)
            _error = true;
        return false;
    }
    ...
    virtual bool visit(BinaryExpressionAST * ast)
    {
        ...
        case T_SLASH:           if (rightResult) _result = leftResult / rightResult; else _error = true; break;
        case T_PERCENT:         if (rightResult) _result = leftResult % rightResult; else _error = true; break;
