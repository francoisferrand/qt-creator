...
QList<FilterEntry> FileSystemFilter::matchesFor(QFutureInterface<Locator::FilterEntry> &future, const QString &entry_)
{
    ...
    const QString lineNoSuffix = ILocatorFilter::extractLineNumber(&entry);
...
void FileSystemFilter::accept(FilterEntry selection) const
{
    ...
    const QString lineNoSuffix = ILocatorFilter::extractLineNumber(&file);
    ...
    m_editorManager->openEditor(selection.internalData.toString(), Id(),
                                Core::EditorManager::ModeSwitch | EditorManager::CanContainLineNumber);
}
